...
| **16** | Instalador e atualizador (quase) visual para self-hosting | **Gatilho DISPAROU** (2026-07-21): o dono quer facilitar instalação/atualização (para si e p/ quem auto-hospedar). Virou o [`plano 025`](plans/025-instalador-e-atualizador-visual.md). Ressalva: num app Node a camada de sistema **não** pode ser 100% web (ovo-e-galinha) — o mínimo é 1 comando; o resto (onboarding/updater) é visual. Modelo: Ghost (CLI provisiona, browser faz onboarding). | **XL** fatiado: Fase 0 (`install.sh`, ~80% do valor) é **M** e independente; wizard e updater visual vêm depois (updater é o risco: auto-restart + migration mão-única) |
