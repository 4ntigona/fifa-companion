...
export default function CapturePage() {
  ...
  const upload = useMutation({
    mutationFn: async (file: File) => {
      if (!aiKey) {
        throw new Error(`Adicione a chave da ${PROVIDER_LABELS[ai.activeProvider]} em Mais › Chaves de IA para analisar fotos.`)
      }
  ...
  return (
    <div className="space-y-4 pt-5">
      ...
      <h1 className="display text-[24px] not-italic text-ink">Captura</h1>
      ...
      <button
        ...
        className="w-full rounded-2xl border-2 border-dashed border-hairline-strong bg-surface-soft py-10 text-center transition-colors hover:border-primary hover:bg-tint-lavender"
      >
        <span className="mx-auto block w-fit text-primary">
          <svg width="34" height="34" viewBox="0 0 22 22" fill="none" aria-hidden="true">
            ...
            <circle cx="11" cy="12" r="3.4" stroke="currentColor" strokeWidth="1.8" />
      ...
      {(analysisError || upload.isError) && (
        <div className="rounded-xl bg-tint-rose p-4 text-sm text-charcoal">
...
function ReviewPanel(props: { extracted: VisionResult; career: Career; onApplied: () => void }) {
  ...
  return (
    <div className="card space-y-3 p-5" style={{ borderLeft: '4px solid var(--color-primary)' }}>
      <div className="flex items-center justify-between">
        <h2 className="display text-[15px] not-italic text-ink">Revisão — {SCREEN_LABEL[extracted.screenType] ?? extracted.screenType}</h2>
      ...
      <ul className="space-y-2">
        {rows.map((row, i) => (
          <li key={i} className={`rounded-xl p-3 text-sm ${row.include ? 'bg-surface-soft' : 'bg-surface-soft opacity-50'}`}>
            <div className="flex items-center gap-2">
              <input type="checkbox" checked={row.include} className="size-4 accent-primary"
