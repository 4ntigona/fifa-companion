...
export default function CareerPage() {
  ...
  const { data: playersData } = useQuery({
    ...
    queryFn: () => listCareerPlayers(Number(id)),
  })
  ...
  const updateSeason = useMutation({
    mutationFn: (payload: { currentSeason?: string; currentDateIngame?: string }) =>
      Promise.resolve(updateCareer(Number(id), payload)),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-8 pt-6">
      ...
      <section className="border border-hairline bg-navy p-6 text-white">
        ...
        <div className="mt-4 flex items-center gap-2  bg-navy-mid/60 p-2.5 text-sm">
          {editingSeason ? (
            ...
            <>
              <span>
                📅 Temporada <b>{career.current_season}</b>
                {career.current_date_ingame ? ` · ${career.current_date_ingame}` : ''}
              </span>
              <button
        ...
        {career.team_type === 'created' && (
          <div className="mt-4 grid grid-cols-2 gap-2 text-sm text-white/80">
            <div>
              Verba: <b className="text-white">{fmtEur(career.created_team_budget_eur)}</b>
            </div>
            <div>
              Liga: <b className="text-white">{career.created_team_league ?? '—'}</b>
            </div>
            <div>
              Qualidade: <b className="text-white">{career.squad_quality ?? '—'}</b>
            </div>
          </div>
        )}
        {career.team && (
          <div className="mt-4 grid grid-cols-2 gap-2 text-sm text-white/80 sm:grid-cols-4">
            <div>
              Geral <b className="text-white">{career.team.overall}</b>
            </div>
            <div>
              ATA <b className="text-white">{career.team.attack}</b>
            </div>
            <div>
              MEI <b className="text-white">{career.team.midfield}</b>
            </div>
            <div>
              DEF <b className="text-white">{career.team.defence}</b>
            </div>
            <div className="col-span-2">
              Verba: <b className="text-white">{fmtEur(career.team.transfer_budget_eur)}</b>
            </div>
          </div>
        )}
        {objectives.length > 0 && (
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Link to={`/carreira/${id}/prospeccao`} className="btn-primary">
            🔎 Prospecção
          </Link>
          <Link
            to={`/carreira/${id}/captura`}
      ...
      <section>
        ...
        {tab === 'elenco' && squad.length === 0 && career.team_type === 'created' && (
          <p className="card bg-surface-soft p-4 text-sm text-slate-ink">
            Clube criado: os jogadores gerados pelo jogo entram por "+ Jogador" (manual) ou por 📸 foto da tela do
            elenco.
          </p>
        )}
      </section>
      ...
      {showAddPlayer && (
        <AddPlayerModal careerId={Number(id)} version={career.fifa_version} onClose={() => setShowAddPlayer(false)} />
      )}
    </div>
  )
}
...
function SeasonEditor(props: {
  ...
  return (
    <div className="flex w-full flex-wrap items-center gap-2">
      ...
      />
      <button
        onClick={() => props.onSave(s, d)}
        disabled={props.saving}
        className="bg-primary px-3 py-1.5 text-[13px] font-semibold text-black hover:bg-primary-pressed hover:text-white"
      >
        Salvar
      </button>
      <button onClick={props.onCancel} className="px-3 py-1.5 text-[13px] font-medium text-white/80 hover:bg-white/10">
        Cancelar
      </button>
    </div>
  )
...
function AddPlayerModal({ careerId, version, onClose }: { careerId: number; version: number; onClose: () => void }) {
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['career-players', String(careerId)] })
      onClose()
    },
  })
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-navy-deep/50 sm:items-center" onClick={onClose}>
      ...
      >
        ...
        <p className="text-[13px] text-steel">
          Para jogadores reais da database do {versionLabel(version)}, use a Prospecção. Aqui entram os que só existem
          no seu save.
        </p>
        <div className="flex gap-2 text-sm">
          {(['youth', 'regen', 'generated'] as const).map((o) => (
            <button key={o} onClick={() => setOrigin(o)} className={origin === o ? 'pill-tab-active' : 'pill-tab'}>
        ...
        <div className="flex gap-2">
          ...
          <input
            ...
            placeholder="Potencial original"
            inputMode="numeric"
            className="input w-1/2"
          />
        </div>
        ...
        <div className="flex gap-2 pt-1">
          <button onClick={() => create.mutate()} disabled={!name || create.isPending} className="btn-primary flex-1">
            Salvar
          </button>
